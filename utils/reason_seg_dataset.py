...
class ReasonSegDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # set is_ood true if the ood_ratio is greater than random number(0-1)
        if self.ood_ratio > random.random():
            is_ood = True
        ...
        if is_ood:
            image_name = image_path.split("/")[-1]
            sents = self.img_to_explanation_ood[image_name]["query"]
        ...
        image_name = image_path.split("/")[-1]
        if self.explanatory != -1 and image_name in self.img_to_explanation:
            if is_ood:  # if ood, choice is 1 or 2
                choice = random.randint(1, 2)
            elif random.random() < self.explanatory:
                choice = 2
            else:
                choice = random.randint(0, 1)
        ...
        for text in sampled_sents:
            ...
            if self.explanatory != -1 and img_name in self.img_to_explanation:
                if is_ood and img_name in self.img_to_explanation_ood:
                    explanation_source = self.img_to_explanation_ood
                else:
                    explanation_source = self.img_to_explanation

                if choice == 0:  # [SEG] token
                    answers.append(random.choice(self.answer_list))
                elif choice == 1:  # [SEG] token + text answer
                    answer = explanation_source[img_name]["outputs"]
                    answer = random.choice(self.answer_list) + " {}".format(answer)
                    questions[-1] = (
                        DEFAULT_IMAGE_TOKEN
                        + "\n"
